...
function fish(shoal){
    ...
    let pond = shoal.split('').sort()
    for(i=0;i<pond.length;i++){
      if(mainFish.level >= parseInt(pond[i])){
        mainFish.xp += parseInt(pond[i])
      }
      if(mainFish.xp >=(4*mainFish.level) + mainFish.total){
        mainFish.total += (4*mainFish.level)
        mainFish.level ++
      }
    }
  
    return mainFish.level
  }
