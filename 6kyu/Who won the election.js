...
function getWinner(listOfBallots) {
    //Your code here
    let counts = {}
    const sorted = listOfBallots.sort()//.map(word => word.includes(' ')? word.split(' ').join('-'):word)
    const average = sorted.length/2
    sorted.forEach(function(char) { ++counts[char] || (counts[char] = 1); });
    console.log(counts)
    let answer = Object.keys(counts).filter(function(x) {if(counts[x]>average) return x})
    return answer[0] || null
  
  }
  
  console.log(getWinner(['Koji Kabuto', 'Koji Kabuto', 'Koji Kabuto', 'Boss', 'Koji Kabuto', 'Koji Kabuto', 'Koji Kabuto', 'Koji Kabuto', 'Koji Kabuto', 'Koji Kabuto']))